...
public class IntLocation implements Comparable<IntLocation> {
	...
	IntLocation( int x , int y , int z , String worldName ) {
		...
		this.y = y;
		this.z = z;
		List<World> worlds = SimplePortal.server.getWorlds();
		int cnt;
	...
	String getWorldName() {
		...
		if(worldName == null) {
			worldName = ((World)SimplePortal.server.getWorlds().get(worldIndex)).getName();
		}
		return worldName;
	}
	...
	static public IntLocation getIntLocation( String string ) {
		...
		if( string == null ) {
			SimplePortal.log( "Unable to parse " + string + " as int location" );
			return null;
		}
		...
		String name;
		if( split.length == 3 ) {
			name = ((World)SimplePortal.server.getWorlds().get(0)).getName();
		} else {
			name = split[3];
		}
